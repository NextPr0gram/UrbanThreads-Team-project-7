                    ...
                    <div class="flex flex-col md:flex-row items-center justify-between p-4 gap-4">
                        ...
                        <div class="flex flex-col md:flex-row items-center w-full md:w-auto">
                            ...
                            <div class="w-24 aspect-square">
                                <img class="w-24 aspect-square rounded-md" src="{{ asset($item->product->image) }}" alt="{{ $item->product->name }}" title="{{ $item->product->name }}">
                            </div>
                        ...
                        <form id="addToBasket" class="flex justify-center items-center gap-4 max-sm:flex-col w-auto md:w-1/2" action="{{ route('basket.add', ['productId' => $item->product->id]) }}" method="post">
                            @csrf
                            @method('POST')
                            {{-- Size Selector --}}
                        ...
                        <form id="removeFromWishlist" action="{{ route('wishlist.remove', $item->product->id) }}" method="post">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="mx-4">
                                <img src="{{ asset('icons/utility/heart-default.svg') }}" class="w-6 h-6" alt="">
                            </button>
                        </form>
                    </div>
                </div>
