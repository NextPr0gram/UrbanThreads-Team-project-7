...
<div class="w-full h-full">
    <div class=" grid grid-cols-2 grid-rows-1 gap-4 h-full w-full px-5  sm:px-8">
                        ...
                        <tr class="h-10">
                            ...
                            <td class="text-left px-1">
                                <form id="status-form-{{ $customerEnquiry->id }}" action="{{ route('status.update', ['enquiryId' => $customerEnquiry->id]) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <x-select name="newStatus" class="my-2 px-full" onchange="submitForm('{{ $customerEnquiry->id }}', this.value)">
        ...
        {{-- Form --}}
        <div id="formOverlay" class="hidden absolute top-0 left-0 lg:hidden w-full h-screen opacity-50 bg-default-black transition-all duration-150 ease-in-out">
        </div>

        {{-- second column --}}
        <div id="editDetails" class="overflow-hidden h-0 md:h-auto translate-y-full lg:w-0 border-none lg:translate-y-0 col-start-3 justify-self-end absolute bottom-0 left-0 border border-neutral-30 rounded-t-lg lg:rounded-lg bg-default-white w-full max-h-3/4  transition-all duration-150 ease-in-out lg:static">
            <div class=" overflow-y-auto py-4 px-5 h-full">
                ...
                <form id="updateProductForm" class="p-0 m-0 lg:flex lg:flex-col lg:h-full" action="" method="POST">
                    ...
                    <div class="flex w-full h-fit">
                        <div class="bg-primary-50 aspect-square w-[9.375rem] sm:w-[9.625rem] md:w-[10.125rem]  rounded-md flex-initial overflow-hidden">
                        ...
                        <div class="w-full flex-1 pl-4 min-w-[12]">
                            ...
                            <x-text-input type="text" id="nameField" name="name" class="w-full " placeholder="Name" />
                            <x-input-label for="price" class="pb-2 pt-4">Price</x-input-label>
                            <x-text-input type="number" id="priceField" name="price" class="w-full " placeholder="Price" />
                        </div>
                    </div>

                    <x-input-label id="StockForSLabel" for="stockForS" class="pb-2 pt-4">Stock For S</x-input-label>
                    <x-text-input type="number" id="StockForSInput" name="stockForS" class="w-full " placeholder="Stock For S" />

                    <x-input-label id="StockForMLabel" for="stockForM" class="pb-2 pt-4">Stock For M</x-input-label>
                    <x-text-input type="number" id="StockForMInput" name="stockForM" class="w-full " placeholder="Stock For M" />

                    <x-input-label id="StockForLLabel" for="stockForL" class="pb-2 pt-4">Stock For L</x-input-label>
                    <x-text-input type="number" id="StockForLInput" name="stockForL" class="w-full " placeholder="Stock For L" />


                    <x-input-label for="description" class="pb-2 pt-4 ">Description</x-input-label>
                    <x-text-area type="text" id="descriptionField" name="description" class="text-base lg:grow w-full h-auto " placeholder="Write your description here" required />
