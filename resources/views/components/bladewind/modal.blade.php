...
<div data-name="{{$name}}" data-backdrop-can-close="{{$backdrop_can_close}}"
    ...
    <div class="sm:{{$sizes[$size]}} w-full p-4 m-auto bw-{{$name}} animate__faster">
        <div class="bg-white dark:border dark:border-slate-800 rounded-lg drop-shadow-2xl">
            ...
            <div class="{{(!empty($type))?'flex':'flex-initial'}}">
                ...
                <div class="modal-body grow p-7 @if(!empty($type)) !pl-3 @endif {{ $body_css  }}">
                    <div class="grid grid-cols-2 justify-between">
                        <h1 class="text-lg font-formula1 text-left text-primary-500">{{ $title }}</h1>
                        <x-secondary-button
                                size="{{$button_size}}"
                                onclick="{!! $cancelAction !!}"
                                class="cancel {{ (($stretch_action_buttons) ? ' w-1/3 mb-3' : '') }} {{ $cancelCss }}">{{$cancel_button_label}}</x-secondary-button>
                    </div>
            ...
            @if( $show_action_buttons )
                <div class="modal-footer @if($stretch_action_buttons) flex flex-col-reverse @endif @if($center_action_buttons || in_array($size, ['tiny', 'small', 'medium'])) text-center @else text-right @endif py-3 px-6 rounded-br-lg rounded-bl-lg {{ $footer_css }}">
                    <x-primary-button
                            size="{{$button_size}}"
                            onclick="{!! $okAction !!}"
                            class="okay {{ (($stretch_action_buttons) ? 'block w-full mb-3 !ml-0' : 'ml-3') }} {{ $okCss }}">{{$ok_button_label}}</x-primary-button>
                </div>
            @endif
        </div>
    </div>
